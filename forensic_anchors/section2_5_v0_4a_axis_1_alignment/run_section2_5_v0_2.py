...
def load_phase1(path: str) -> pd.DataFrame:
    ...
    T10 fix (BOM strip): phase1 CSV may have UTF-8 BOM-prefixed header
    column ('\\ufeffgalaxy'); strip BOM during normalization.
    """
...
def load_MRT(path: str) -> pd.DataFrame:
    """
    SPARC MRT (Lelli+2016 standard, 175 galaxies, ~98-line header).

    T11 fix: actual MRT data starts at line 99 (skiprows=98). Use
    whitespace-delimited parsing to handle variable-width Galaxy field.

    Whitespace-split column index mapping (per MRT byte-by-byte description):
        idx 0 : Galaxy
        idx 1 : T (Hubble type)
        idx 2 : D (distance, Mpc)
        idx 3 : e_D
        idx 4 : f_D (distance method)
        idx 5 : Inc
        idx 6 : e_Inc
        idx 7 : L[3.6] (10^9 solLum)  -> L36
        idx 8 : e_L[3.6]
        idx 9 : Reff
        idx 10: SBeff
        idx 11: Rdisk (kpc)
        idx 12: SBdisk -> SBdisk0
        idx 13: MHI (10^9 solMass)
        idx 14: RHI
        idx 15: Vflat (km/s)
        idx 16: e_Vflat
        idx 17: Q (quality flag)
        idx 18: Reference (last column)

    Required output columns: galaxy, T, Rdisk, SBdisk0, L36, MHI, Vflat
    """
    # All raw columns (before subset); use names with idx mapping above.
    raw_names = [
    ...
    df = pd.read_csv(
        ...
        header=None,
        names=raw_names,
        usecols=range(len(raw_names)),  # ignore Reference column (variable text)
        engine="python",
    )
...
def _prepare_dsph_phase_c3_sample(
    ...
    """
    df = dsph_df.copy()
    # Derive rho_gal + r_h from input dsph_jeans_c15_v1.csv columns:
    #   - LV (luminosity, L_sun) + Y_V=1 convention -> M_bar = LV
    #   - rh_pc (half-light radius, pc) / 1000 -> r_h_kpc
    #   - rho_gal = M_bar / (4/3 π r_h_kpc^3)  [M_sun / kpc^3]
    # Source: phase_c3_step3 docstring L17-19 (T18 verbatim).
    if "rho_gal" not in df.columns or "r_h" not in df.columns:
        if "LV" in df.columns and "rh_pc" in df.columns:
            df["M_bar"] = df["LV"]                        # Y_V=1 convention
            df["r_h"]  = df["rh_pc"] / 1000.0             # pc -> kpc
            df["rho_gal"] = df["M_bar"] / (
                4.0 / 3.0 * np.pi * df["r_h"] ** 3
            )
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if not args.skip_self_check:
        ...
        if rotation_curves_by_galaxy:
            # Phase C3 v3 §4.3 / phase_c3_step3 g_obs convention (T16.F verbatim
            # L243-250): g_obs_C2 = mean(V_obs² / r) for r > 2·hR, in
            # (km/s)²/kpc units. This matches gc_C15 unit convention via
            # A0_KPC = a_0·3.086e19/1e6 ≈ 3.7e3 (km/s)²/kpc, so the
            # log10(g_obs/gc_C15) ratio is dimensionally consistent for
            # b_α partial OLS estimator.
        ...
        # Adapter: sparc_171 columns 'galaxy' / 'Ud' (or 'ud') / 'L36' / 'MHI' /
        # 'Rdisk' / 'Vflat' need to be aliased to b_alpha audit's expected schema
        # ('Galaxy' / 'Upsilon_d' / 'L36' / 'MHI' / 'Rdisk' / 'Vflat').
        # load_phase1 produces 'Ud' (capital), some legacy callers use 'ud'.
        col_alias_map = {
            "galaxy": "Galaxy", "Ud": "Upsilon_d", "ud": "Upsilon_d",
        }
