...
v4.8 v1 版で採用した $\fopt(0.83) = 1.9163$, $\vflat^{\mathrm{universal}} = 2.00 \times 10^{5}$ m/s,
$\cmem(0.83) = 3.833 \times 10^{5}$ m/s の 3 量は NGC 3198 ($c=0.42$) と NGC 2841 ($c=0.80$) の
2 点線形外挿 artifact であり, v3.7 Chap 18 の独立定義として存在するものではない.
本 v2 erratum ではこの ``universal'' 表現を撤回し, per-galaxy 記述に戻す (patch spec v2 \S 6.3 /
retract 不可 item \#41). $c_0 = 0.83$ での 3 量は 2 点外挿の補助値として Appendix A でのみ参照する.
外挿精度は \S 7.3 の caveat を参照.
...
v4.8 v1 版で採用した $\Lambdauv(c_0 = 0.83) = 9.549 \times 10^{-49}$ J は NGC 3198 と NGC 2841 の 2 点
$\msigma$ 線形外挿で得られた補助値であり, v3.7 Chap 18 Table 18-4 に独立項目として存在するものではない.
本 v2 erratum ではこの ``universal $\Lambdauv(0.83)$'' 表現を撤回する (patch spec v2 \S 6.3, retract 不可 \#41).
$c_0 = 0.83$ での値は Appendix A で 2 点外挿の補助値としてのみ参照する (\S 7.3 caveat 参照).
...
$\aPT^{\mathrm{upper}} = \BFIRAS/(\kmem\cdot\Nmode)$. v4.8 v1 版の $\Vxi(0.83) = 7.683\times10^{63}\;\mathrm{m}^3$ と
$\aPT^{\mathrm{upper}} = 2.96\times10^{-53}$ は NGC 3198 ($c=0.42$) + NGC 2841 ($c=0.80$) の 2 点外挿 artifact で,
NGC 3198 に v37 convention $\vflat=119$ km/s を用いた値. 本 v2 erratum の MRT 統一下では
$\cmem(\mathrm{NGC~3198}) = 1.516\times10^5$ m/s, $\aPT^{\mathrm{upper}}(\mathrm{NGC~3198}, \Vxi) = 1.76\times10^{-51}$
(v4.8 v1 から factor 59 弱い上限). 3 銀河 range は $[1.6\times10^{-53},\, 1.8\times10^{-50}]$ (≈3 桁幅).)
\normalsize
...
\subsection{$\Lambdauv$ および $\cmem$ 外挿精度 --- artifact 自認}
v4.8 v1 版で採用した $\cmem(0.83) = 3.833 \times 10^{5}$ m/s, $\Lambdauv(0.83) = 9.549 \times 10^{-49}$ J,
および $\Vxi(0.83) = 7.683 \times 10^{63}\;\mathrm{m}^3$, $\aPT^{\mathrm{upper}}(\Vxi) = 2.96 \times 10^{-53}$ は,
v3.7 Chap 18 Table 18-3 / 18-4 の NGC 3198 ($c=0.42$) + NGC 2841 ($c=0.80$) の 2 点線形外挿に依存する補助値であり,
v3.7 Chap 18 の独立定義として存在するものではない. これらを ``universal'' と呼ぶことは, patch spec v2 \S 6.3
(retract 不可 item \#41) に照らし不適切である.

本 v2 erratum では, \S 3.1 / \S 3.3 / \S 4.2 および Appendix A で per-galaxy (IC 2574 / NGC 3198 / NGC 2841)
表現に修正した. $c_0 = 0.83$ での補助値は外挿由来として Appendix A で明示される.

追加課題:
\begin{itemize}
\item v4.8 v1 版の $\aPT^{\mathrm{upper}} = 2.96 \times 10^{-53}$ ($\Vxi$ primary) は v37 convention
  $\cmem(\mathrm{NGC~3198}) = 1.194 \times 10^5$ m/s ($\vflat = 119$ km/s, v3.7 Chap 18 Table 18-3 由来)
  から導出されたもの. 本 v2 erratum で採用した MRT 統一 ($\vflat = 150.1$ km/s, Lelli 2016 準拠) では
  $\cmem(\mathrm{NGC~3198}) = 1.516 \times 10^5$ m/s, $\aPT^{\mathrm{upper}}(\mathrm{NGC~3198}, \Vxi) = 1.76 \times 10^{-51}$
  (factor 59 弱い上限). 3 銀河 range は $[1.6 \times 10^{-53},\, 1.8 \times 10^{-50}]$ (≈3 桁幅).
\item 主張 M3 の ``NGC 3198 2.32\% PASS'' は numerical product $\kmem\cdot\Nmode$ と hand-derived target との
  internal rounding check (両者 v37 anchor values 由来). Symbolic analysis で
  \[
  \kmem \cdot \Nmode\bigr|_{\Vxi}
  = \frac{2}{9\pi}\,\frac{\cmem^{2}\, \Lambdauv^{2}}{a_0^{5/2}\,\hbar^{3}}
  \]
  という closed-form identity が成立し, numerical product と machine precision で一致 (ratio = 1.0000).
  2.32\% 残差は v37 hand-compute target の rounding であり physical prediction precision ではない.
  本 v2 では closed-form identity verification として reframe し, PASS/FAIL physical test としては撤回する.
\item $\fopt(c)$ の deg-4 5 点 fit (v3.7 Table 18-3 の $c \in \{0.30, 0.42, 0.618, 0.80, 1.00\}$ 5 点) 採用時,
  $\fopt(0.83) = 1.9163 \to 1.9425$ (+1.37\%) の numerical drift が予想される. cascade により
  $\cmem(0.83)$, $\Vxi(0.83)$, $\aPT^{\mathrm{upper}}$ が同様 drift する. 本 update は v3 erratum または
  v4.9 cycle で反映する.
\item $\cmem(c)$ の追加銀河サンプルと $\Lambdauv(c)$ の独立測定 (CMB spectral distortion 精密化,
  \citet{Kogut2011} PIXIE) が外挿精度向上の根本解決.
\end{itemize}
...
$\cmem(0.83)^{\dagger}$ & $3.833 \times 10^{5}$ m/s & 2-pt artifact (NGC3198+NGC2841 外挿, v37 convention) \\
$\chi_F(0.83)^{\dagger}$ & $4.198\;\mathrm{m}^{3/2}/\mathrm{s}^2$ & 2-pt artifact (Eq. 2A-II derivative) \\
$V''(\phi_0, x=0)$ & $2.31$ & Chap 18 Q1-$\alpha$ \\
$\Lambdauv(0.83)^{\dagger}$ & $9.549 \times 10^{-49}$ J & 2-pt artifact (Table 18-4 外挿) \\
...
\footnotesize\noindent
$^{\dagger}$ 2-pt artifact: NGC 3198 ($c=0.42$) + NGC 2841 ($c=0.80$) の 2 点線形外挿で得られた補助値.
v3.7 Chap 18 Table 18-3/18-4 の独立定義として存在するものではない. 詳細は \S 7.3 および companion
document \texttt{c3\_contamination\_dependency\_map\_v1} 参照.
\normalsize
