...
# �� MSZ + Duffy+2008 �X�VNFW�p�����[�^
CLUSTERS={
    'J0201':{'ra':30.652,'dec':-4.955,'z_l':0.196,'r_s':0.169,'r500':0.676,
             'rho_s':4.8e14,'src':'J0201_src.csv','type':'Miyaoka'},
    'J0231':{'ra':37.941,'dec':-4.762,'z_l':0.184,'r_s':0.190,'r500':0.760,
             'rho_s':4.2e14,'src':'J0231_src.csv','type':'Miyaoka'},
    'J1311':{'ra':197.863,'dec':0.403,'z_l':0.183,'r_s':0.246,'r500':0.985,
             'rho_s':3.5e14,'src':'J1311_src.csv','type':'Miyaoka'},
    'J2337':{'ra':354.378,'dec':-0.022,'z_l':0.282,'r_s':0.238,'r500':0.952,
             'rho_s':3.6e14,'src':'J2337_src.csv','type':'Miyaoka'},
    'G167' :{'ra':33.684,'dec':-4.586,'z_l':0.139,'r_s':0.2934,'r500':0.6458,
             'rho_s':2.164e15,'src':'psz2_PSZ2_G167.98-59.95_src.csv','type':'PSZ2_v2'},
    'G273' :{'ra':180.109,'dec':3.348,'z_l':0.134,'r_s':0.4499,'r500':1.220,
             'rho_s':1.806e15,'src':'psz2_PSZ2_G273.59+63.27_src.csv','type':'PSZ2_v2'},
    'G286' :{'ra':185.703,'dec':2.116,'z_l':0.229,'r_s':0.2886,'r500':0.8205,
             'rho_s':2.226e15,'src':'psz2_PSZ2_G286.39+64.06_src.csv','type':'PSZ2_v2'},
    'G223' :{'ra':131.371,'dec':3.475,'z_l':0.327,'r_s':0.3206,'r500':0.9041,
             'rho_s':2.202e15,'src':'psz2_PSZ2_G223.47+26.85_src.csv','type':'PSZ2_v2'},
    'G228' :{'ra':140.561,'dec':3.753,'z_l':0.270,'r_s':0.3334,'r500':0.9343,
             'rho_s':2.122e15,'src':'psz2_PSZ2_G228.50+34.95_src.csv','type':'PSZ2_v2'},
    'G231' :{'ra':139.058,'dec':-0.412,'z_l':0.332,'r_s':0.3100,'r500':0.8893,
             'rho_s':2.237e15,'src':'psz2_PSZ2_G231.79+31.48_src.csv','type':'PSZ2_v2'},
}

results={}
print(f'{"�N���X�^�[":8s} {"�^�C�v":10s} {"N":8s} {"Q":8s} {"����":6s} {"ki":9s} {"ko":9s}')
for cn,cl in CLUSTERS.items():
    try: df=pd.read_csv(cl['src'])
    except: print(cn,'CSV�Ȃ�'); continue
    ...
    gt_o=np.full(len(Rm),np.nan); gt_n=np.full(len(Rm),np.nan)
    ...
    kG=gt_o-gt_n
    inner=(Rm<rs)&np.isfinite(kG); outer=(Rm>=rs)&(Rm<r5)&np.isfinite(kG)
    ...
    print(f'{cn:8s} {cl["type"]:10s} {len(df):8d} '
    ...
    results[cn]={'Q':Q,'ki':ki,'ko':ko,'type':cl['type']}

print()
valid_Q=[r['Q'] for r in results.values() if np.isfinite(r.get('Q',np.nan)) and r['Q']>0]
print(f'�S�T���v�� N={len(valid_Q)}  ����Q={np.mean(valid_Q):.3f}  Q>1: {sum(1 for q in valid_Q if q>1)}��')
...
print()
# G167�iko?0�j�EG286�i����Ȃ��j�EJ0231�i�s����j���O
main_Q={cn:r['Q'] for cn,r in results.items()
        if cn not in ['G167','G286','J0231'] and np.isfinite(r.get('Q',np.nan)) and r['Q']>0}
print(f'���́iG167/G286/J0231���O�jN={len(main_Q)}  ����Q={np.mean(list(main_Q.values())):.3f}')
t2,p2=stats.ttest_1samp(list(main_Q.values()),1.0)
print(f't���� p={p2:.4f}')
try:
    st2,pw2=stats.wilcoxon([q-1 for q in main_Q.values()],alternative='greater')
    print(f'Wilcoxon p={pw2:.4f}')
except: pass
