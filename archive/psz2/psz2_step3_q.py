...
def calc_Q(df, cl, sigma_cut=3.0):
    ...
    for i,(r1,r2) in enumerate(zip(Rb[:-1],Rb[1:])):
        ...
        gt_raw=np.average(DS[inn],weights=w[inn])/Scr_m
        # sigma_cut: ビン内個別galaxy et の外れ値除去
        et_bin=et[inn]; w_bin=w[inn]
        et_mean=np.average(et_bin,weights=w_bin)
        et_std=np.sqrt(np.average((et_bin-et_mean)**2,weights=w_bin))
        keep=np.abs(et_bin-et_mean)<sigma_cut*et_std
        if keep.sum()<5: continue
        gt_o[i]=np.average(DS[inn][keep],weights=w_bin[keep])/Scr_m
...
for cn,cl in CLUSTERS.items():
    ...
    if len(df)<100: print(cn,'N不足'); continue
    Q,ki,ko,kg=calc_Q(df,cl,sigma_cut=3.0)
    mm=df['shear_m'].mean()
...
print(f'有効Q値: {len(valid_Q)}件  平均Q={np.mean(valid_Q):.3f}')
t,p=stats.ttest_1samp(valid_Q,1.0)
print(f't検定   H0:Q=1  t={t:.3f}  p={p:.4f}')
try:
    stat,pw=stats.wilcoxon([q-1 for q in valid_Q],alternative='greater')
    print(f'Wilcoxon H0:Q<=1  stat={stat:.1f}  p={pw:.4f}')
except: pass

# SNR>=5クラスターのみでも確認
SNR={'J0201':3.1,'J0231':2.4,'J1311':3.0,'J2337':2.1,
     'G167':7.0,'G273':12.6,'G286':4.9,'G223':5.0,'G228':6.7,'G230':5.2,'G231':4.9}
hi_snr=[cn for cn in results if SNR.get(cn,0)>=5.0 and np.isfinite(results[cn]['Q'])]
Qhi=[results[cn]['Q'] for cn in hi_snr]
print(f'SNR>=5クラスター: {hi_snr}')
print(f'Q値: {[round(q,3) for q in Qhi]}')
if len(Qhi)>=3:
    t2,p2=stats.ttest_1samp(Qhi,1.0)
    print(f't検定(SNR>=5)  N={len(Qhi)}  平均Q={np.mean(Qhi):.3f}  t={t2:.3f}  p={p2:.4f}')
    try:
        st2,pw2=stats.wilcoxon([q-1 for q in Qhi],alternative='greater')
        print(f'Wilcoxon(SNR>=5)  stat={st2:.1f}  p={pw2:.4f}')
    except: pass
