# -*- coding: utf-8 -*-
"""
Step 5 v2.4: 自己無撞着 r_s モデル
rs = (v_e² + v_p²·ln2) / g_c
v² = v_bar² + v_e²·(1-exp(-r/rs)) + v_p²·ln(1+r/rs)

使用例:
  python -X utf8 step5_v24.py --rotmod-dir Rotmod_LTG
"""
...
# モデル
# ============================================================
def rs_selfconsistent(v_e, v_p):
    """rs = (v_e² + v_p²·ln2) / g_c [kpc]"""
    v_eff2 = (v_e * 1e3)**2 + (v_p * 1e3)**2 * np.log(2)
    return max(v_eff2 / G_c / kpc_to_m, 0.01)

def v_model_v24(r_kpc, v_e, v_p, v_disk, v_gas, v_bul):
    rs = rs_selfconsistent(v_e, v_p)
...
def fit_v24(df):
    ...
    def model(r, v_e, v_p):
        return v_model_v24(r, v_e, v_p, vd, vg, vb)

    best_chi2 = np.inf
    best_popt = None
    for ve0 in [20, 50, 80, 120, 180, 250]:
        for vp0 in [0, 10, 30, 60, 100, 150]:
            try:
                popt, _ = curve_fit(
                    model, r, vobs,
                    p0=[ve0, vp0],
...
def plot_galaxy(df, name, result, vft, rst, ct, ax):
    ...
    vm   = v_model_v24(rp, v_e, v_p, vdi, vgi, vbi)
    ...
    ax.plot(rp, vm,    'r-',  lw=2.0, label=f'v2.4 chi2={result["chi2dof"]:.2f}')
    ...
    ax.set_title(f"{name}  ve={v_e:.0f} vp={v_p:.0f} pr={pr:.2f}  "
                 f"rs={rs:.1f}  chi2={result['chi2dof']:.2f}", fontsize=9)
...
def main():
    ...
    print(f"{'galaxy':15s} {'v_e':>6} {'v_p':>6} {'vflat':>6} {'pr':>5} "
          f"{'rs_v24':>7} {'rs_th':>7} {'chi2_24':>8} {'chi2_th':>8} {'dAIC':>7}")
    print("-" * 90)
    ...
    for name in targets:
        ...
        n = len(df)
        daic = result['chi2_total'] - ct * max(n-2, 1) if not np.isnan(ct) else np.nan

        pr = result['plastic_ratio']
        rst_str = f"{rst:7.2f}" if not np.isnan(rst) else "    N/A"
        ...
        print(f"{name:15s} {result['v_e']:6.1f} {result['v_p']:6.1f} "
              f"{result['vflat']:6.1f} {pr:5.2f} {result['rs']:7.2f} "
              f"{rst_str} {result['chi2dof']:8.3f} {ct_str} {da_str}")
        ...
        records.append({
            ...
            'rs_v24':         round(result['rs'], 2),
            'rs_tanh':        round(rst, 2) if not np.isnan(rst) else np.nan,
            'chi2dof_v24':    round(result['chi2dof'], 3),
    ...
    df_res.to_csv('v24_results.csv', index=False, encoding='utf-8-sig')
    ...
    plt.tight_layout()
    plt.savefig('v24_multifit.png', dpi=150, bbox_inches='tight')

    # サマリー
    print(f"\n{'='*100}")
    print("v2.4 サマリー（自己無撞着 rs）")
    print(f"{'='*100}")
    print(df_res[['galaxy','v_e','v_p','vflat','plastic_ratio',
                  'rs_v24','rs_tanh','chi2dof_v24','chi2dof_tanh','dAIC']].to_string(index=False))

    # rs 比較
    print(f"\n--- rs_v24 vs rs_tanh ---")
    valid_rs = df_res.dropna(subset=['rs_tanh'])
    if len(valid_rs) > 0:
        for _, row in valid_rs.iterrows():
            ratio = row['rs_v24'] / row['rs_tanh'] if row['rs_tanh'] > 0 else np.nan
            match = "MATCH" if (not np.isnan(ratio) and 0.3 < ratio < 3.0) else ""
            print(f"  {row['galaxy']:15s}: rs_v24={row['rs_v24']:6.2f}, "
                  f"rs_tanh={row['rs_tanh']:6.2f}, ratio={ratio:.2f}  {match}"
                  if not np.isnan(ratio) else
                  f"  {row['galaxy']:15s}: rs_v24={row['rs_v24']:6.2f}, rs_tanh=N/A")
    ...
    for _, row in df_res.iterrows():
        ...
        parts = []
        if c24 < 2:    parts.append(f"chi2={c24:.2f} OK")
        elif c24 < 5:  parts.append(f"chi2={c24:.1f} marginal")
        else:          parts.append(f"chi2={c24:.0f} NG")

        if not np.isnan(daic):
            if abs(daic) < 2:   parts.append("~tanh")
            elif daic < 0:      parts.append(f"dAIC={daic:.0f} v24 WINS")
            else:               parts.append(f"dAIC={daic:.0f}")

        if pr > 0.01:  parts.append(f"pr={pr:.2f} PLASTIC")
        else:          parts.append(f"pr={pr:.2f} elastic-only")
        print(f"  {name:15s}: {', '.join(parts)}")

    print(f"\n図を v24_multifit.png に保存しました。")
