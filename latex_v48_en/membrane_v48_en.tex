...
The three quantities adopted in v4.8 v1---$\fopt(0.83) = 1.9163$,
$\vflat^{\mathrm{universal}} = 2.00 \times 10^{5}$ m/s, and $\cmem(0.83) = 3.833 \times 10^{5}$
m/s---are two-point linear extrapolation artifacts depending on NGC 3198 ($c=0.42$) and
NGC 2841 ($c=0.80$), and do not exist as independent definitions in v3.7 Chap.~18. This
v2 erratum withdraws the ``universal'' expression and returns to per-galaxy description
(patch spec v2 \S 6.3 / non-retractable item \#41). Values at $c_0 = 0.83$ are referenced only
as auxiliary two-point extrapolations in Appendix~A. Extrapolation accuracy is discussed in
the caveat of \S 7.3.
...
The value $\Lambdauv(c_0 = 0.83) = 9.549 \times 10^{-49}$ J adopted in v4.8 v1 is an auxiliary
two-point $\msigma$ linear extrapolation from NGC 3198 and NGC 2841, and does not exist as an
independent entry in v3.7 Chap.~18 Table~18-4. This v2 erratum withdraws the ``universal
$\Lambdauv(0.83)$'' expression (patch spec v2 \S 6.3, non-retractable \#41). The value at
$c_0 = 0.83$ is referenced only as a two-point extrapolation auxiliary in Appendix~A
(see caveat in \S 7.3).
...
$\aPT^{\mathrm{upper}} = 2.96\times10^{-53}$ were two-point extrapolation artifacts from
NGC 3198 ($c=0.42$) + NGC 2841 ($c=0.80$) with v37-convention $\vflat=119$ km/s for NGC 3198.
Under the MRT unification of this v2 erratum, $c_{\mathrm{mem}}(\mathrm{NGC~3198}) = 1.516\times10^5$ m/s
and $\aPT^{\mathrm{upper}}(\mathrm{NGC~3198}, V_\xi) = 1.76\times10^{-51}$---factor 59 weaker
than v4.8 v1. The 3-galaxy range is $[1.6\times10^{-53},\, 1.8\times10^{-50}]$
...
\subsection{Extrapolation accuracy of $\Lambdauv$ and $\cmem$ --- self-identified artifact}
The four quantities adopted in v4.8 v1---$\cmem(0.83) = 3.833 \times 10^{5}$ m/s,
$\Lambdauv(0.83) = 9.549 \times 10^{-49}$ J, $\Vxi(0.83) = 7.683 \times 10^{63}$ m$^3$, and
$\aPT^{\mathrm{upper}}(V_\xi) = 2.96 \times 10^{-53}$---are auxiliary values depending on
two-point linear extrapolation from NGC 3198 ($c=0.42$) and NGC 2841 ($c=0.80$) in
v3.7 Chap.~18 Table~18-3 / 18-4, and do not exist as independent definitions in
v3.7 Chap.~18. Labeling them as ``universal'' is inappropriate in light of patch spec v2 \S 6.3
(non-retractable item \#41).

In this v2 erratum, \S 3.1 / \S 3.3 / \S 4.2 and Appendix~A have been revised to per-galaxy
expressions (IC 2574 / NGC 3198 / NGC 2841). Values at $c_0 = 0.83$ are explicitly indicated
as extrapolation-derived in Appendix~A.

Additional items:
\begin{itemize}
\item The v4.8 v1 bound $\aPT^{\mathrm{upper}} = 2.96 \times 10^{-53}$ ($V_\xi$ primary)
      was derived using v37-convention $\cmem(\mathrm{NGC~3198}) = 1.194 \times 10^5$ m/s
      (from $\vflat = 119$ km/s, v3.7 Chap.~18 Table~18-3). The MRT unification adopted in this
      v2 erratum ($\vflat = 150.1$ km/s per Lelli 2016) gives
      $\cmem(\mathrm{NGC~3198}) = 1.516 \times 10^5$ m/s and
      $\aPT^{\mathrm{upper}}(\mathrm{NGC~3198}, V_\xi) = 1.76 \times 10^{-51}$---factor 59
      weaker. The 3-galaxy range is $[1.6 \times 10^{-53},\, 1.8 \times 10^{-50}]$
      ($\approx$3 orders of magnitude).
\item The ``NGC 3198 2.32\% PASS'' claim for M3 is an internal rounding check between the
      numerical product $\kmem\cdot\Nmode$ and a hand-derived target, both computed from v37
      anchor values. Symbolic analysis yields the closed-form identity
      \[
      \kmem \cdot \Nmode\bigr|_{V_\xi}
      = \frac{2}{9\pi}\,\frac{\cmem^{2}\, \Lambdauv^{2}}{a_0^{5/2}\,\hbar^{3}},
      \]
      which is identical to the numerical product to machine precision (ratio $= 1.0000$).
      The 2.32\% residual measures rounding of the hand-computed target rather than a physical
      prediction precision. In this v2 the claim is reframed as a closed-form identity
      verification, not a PASS/FAIL physical test.
\item A numerical drift is anticipated upon adoption of the deg-4 5-point fit of $\fopt(c)$
      (v3.7 Table~18-3, $c \in \{0.30, 0.42, 0.618, 0.80, 1.00\}$ five points):
      $\fopt(0.83) = 1.9163 \to 1.9425$ (+1.37\%). A cascade will drift $\cmem(0.83)$,
      $\Vxi(0.83)$, and $\aPT^{\mathrm{upper}}$ similarly. This update will be reflected in a v3
      erratum or the v4.9 cycle.
\item Additional galaxy samples for $\cmem(c)$ and independent measurements of $\Lambdauv(c)$
      (precision CMB spectral distortion measurements, PIXIE~\citep{Kogut2011}) provide the
      fundamental solution for extrapolation accuracy.
\end{itemize}
...
$\cmem(0.83)^{\dagger}$ & $3.833 \times 10^{5}$ m/s & 2-pt artifact (NGC3198+NGC2841 extrap., v37 conv.) \\
$\chi_F(0.83)^{\dagger}$ & $4.198\;\mathrm{m}^{3/2}/\mathrm{s}^2$ & 2-pt artifact (Eq.~2A-II derivative) \\
$V''(\phi_0, x=0)$ & $2.31$ & Chap.~18 Q1-$\alpha$ \\
$\Lambdauv(0.83)^{\dagger}$ & $9.549 \times 10^{-49}$ J & 2-pt artifact (Table 18-4 extrap.) \\
$\Vxi(0.83)^{\dagger}$ & $7.683 \times 10^{63}\;\mathrm{m}^3$ & 2-pt artifact (cascade) \\
...
\footnotesize\noindent
$^{\dagger}$ 2-pt artifact: auxiliary value from two-point linear extrapolation of NGC 3198
($c=0.42$) + NGC 2841 ($c=0.80$). Does not exist as an independent definition in v3.7 Chap.~18
Table~18-3/18-4. See \S 7.3 and companion document
\texttt{c3\_contamination\_dependency\_map\_v1} for details.
\normalsize
