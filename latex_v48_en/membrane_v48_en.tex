...
$\aPT^{\mathrm{upper}}$ (phase-transition coupling upper bound) as a per-galaxy evaluation across
three anchor galaxies (IC 2574 / NGC 3198 / NGC 2841), with the representative value
$\aPT^{\mathrm{upper}}(\mathrm{NGC\,3198}, \Vxi) = 1.76 \times 10^{-51}$ (MRT-unified anchor).
The value $2.96 \times 10^{-53}$ in v4.8 v1 was a two-point extrapolation artifact from the v37
convention; it is factor 59 tighter and is superseded in this v2 erratum (see \S 7.3).
The 3-galaxy range spans $[1.6 \times 10^{-53},\, 1.8 \times 10^{-50}]$ ($\approx$3 orders of magnitude).
...
The numerical evaluation of $\cmem$ uses the galaxy-by-galaxy $\fopt(c) \cdot \vflat$ mapping of
the v3.7 Chap.~18 Table~18-3. The following are per-galaxy values for the three anchor galaxies
(IC 2574 / NGC 3198 / NGC 2841) under MRT-unified $\vflat$ (Lelli 2016):
...
The value $\fopt(0.83) = 1.9163$ adopted in v4.8 v1 was a two-point $\fopt$-linear extrapolation
artifact from NGC 3198 ($c=0.42, \fopt=1.01$) and NGC 2841 ($c=0.80, \fopt=1.85$)
(patch spec v2 \S 6.3 / non-retractable \#41). This v3 erratum applies a deg-4 exact-pass
Lagrange fit to $V''(x=0.5, c)$ over the five points
$c \in \{0.30, 0.42, 0.618, 0.80, 1.00\}$ from v3.7 Chap.~18 Table~18-2, yielding
$V''(x=0.5, 0.83) = 10.463$ and, through the closed form $\fopt = 2\pi/\sqrt{V''(x=0.5,c)}$,
the confirmed value $\fopt(0.83) = 1.9425$ ($+1.37\%$ from v1). The cascade gives
$\cmem(0.83) = 3.885 \times 10^{5}$ m/s ($+1.36\%$),
$\chi_F(0.83) = 4.256\;\mathrm{m}^{3/2}/\mathrm{s}^2$ ($+1.38\%$),
$\Vxi(0.83) = 8.335 \times 10^{63}\;\mathrm{m}^3$ ($+8.49\%$). These deg-4-confirmed
universal values are tabulated in Appendix~A. Methodology details and comparison with v1 (2-pt
$\fopt$-linear) are in \S 7.3.
...
\end{keybox}
The following are per-galaxy $\Lambdauv$ values from v3.7 Table~18-4:
...
The $\msigma$ decomposition F2 from v3.7: $\msigma = \sqrt{V''(x=0.5, c)}/\tau_{\mathrm{dyn}}(\mathrm{galaxy})$
shows that $\tau_{\mathrm{dyn}}$ is galaxy-specific, so $\Lambdauv = \msigma \cdot \clt^2$ inherently
contains a galaxy-dependent factor. Therefore ``universal $\Lambdauv(c)$'' does not exist in
v3.7 and cannot be presented as a c-only universal quantity consistent with F2.

v4.8 v1 presented $\Lambdauv(0.83) = 9.549 \times 10^{-49}$ J as a two-point $\msigma$ linear
extrapolation universal value. This v3 erratum \textbf{completely retracts universal $\Lambdauv$}
(patch spec v2 \S 6.3, non-retractable \#41, v3 structural finding \S 4.2). Hereafter the only
legitimate $\Lambdauv$ numerical anchors are the three per-galaxy values in the table above.
The universal $\Lambdauv(0.83)$ row has also been removed from Appendix~A. Retract rationale
details are in \S 7.3.
...
\end{center}
\footnotesize
(All three rows use MRT-unified $\vflat$ from Lelli 2016 and $\msigma$ from v3.7 Chap.~18
Table~18-4; $c_{\mathrm{mem}} = \fopt(c)\cdot\vflat$; $V_\xi = (4\pi/3)(c_{\mathrm{mem}}^2/a_0)^3$;
$\alpha_{\mathrm{PT}}^{\mathrm{upper}} = \BFIRAS/(k_{\mathrm{mem}}\cdot N_{\mathrm{mode}})$.
The v4.8 v1 values $V_\xi(0.83) = 7.683\times10^{63}\;\mathrm{m}^3$ and
$\aPT^{\mathrm{upper}} = 2.96\times10^{-53}$ were two-point $\fopt$-linear extrapolation artifacts from
NGC 3198 ($c=0.42$) + NGC 2841 ($c=0.80$) with v37-convention $\vflat=119$ km/s for NGC 3198.
v2 erratum established the MRT-unified per-galaxy anchor
$\aPT^{\mathrm{upper}}(\mathrm{NGC~3198}, V_\xi) = 1.76\times10^{-51}$---factor 59 weaker
than v4.8 v1. v3 erratum (this version) updates universal $\Vxi(0.83)$ to the deg-4 5-pt fit value
$8.335\times10^{63}\;\mathrm{m}^3$ (Appendix~A). The 3-galaxy range remains
$[1.6\times10^{-53},\, 1.8\times10^{-50}]$ ($\approx$3 orders of magnitude).)
\normalsize

\medskip\noindent
Adopting $V_{\mathrm{cosmo}}$ (geometric) gives $\sim 10^{15}$ times a conservative upper bound,
but overestimates physical coherence and is not adopted as primary. Adopting $\Vxi$ as primary
is the only choice consistent with the ``$a_0$ independence'' structural finding of
foundation\_$\alpha$2.

\begin{keybox}
\textbf{M1 claim (v2 revised):} $\aPT^{\mathrm{upper}}$ is evaluated per-galaxy, with
$\aPT^{\mathrm{upper}}(\mathrm{NGC\,3198}, \Vxi) = 1.76 \times 10^{-51}$ as the MRT-unified
anchor value. The representative range across three galaxies is
$\aPT^{\mathrm{upper}} \in [1.6\times10^{-53},\, 1.8\times10^{-50}]$ ($\approx$3 orders of magnitude).
The v4.8 v1 bound $2.96 \times 10^{-53}$ was derived from v37-convention
$\vflat(\mathrm{NGC~3198}) = 119$ km/s; the updated bound under MRT unification
($\vflat = 150.1$ km/s) is factor 59 weaker.
\end{keybox}
...
\subsection{v3 erratum: deg-4 $\fopt$ confirmation + universal $\Lambdauv$ complete retraction}
This v3 erratum is a structural cleanup second pass relative to v2, leaving the per-galaxy
primary framework (M1, M2, M3 all confirmed in v2) entirely unchanged. It is an additive update.

\subsubsection*{(a) $\fopt(0.83)$ deg-4 5-point fit confirmation}

v4.8 v1 adopted $\fopt(0.83) = 1.9163$ via a two-point $\fopt$-linear extrapolation from
NGC 3198 ($c=0.42, \fopt=1.01$) and NGC 2841 ($c=0.80, \fopt=1.85$), identified as an artifact.
The v3.7 Chap.~18-3 closed form $\fopt(c) = 2\pi/\sqrt{V''(x=0.5, c)}$ applied to the 5-point
Table~18-2 $V''(x=0.5, c)$ data:
...
Applying a deg-4 exact-pass Lagrange polynomial fit to $V''(x=0.5, c)$ and evaluating at
$c=0.83$ yields $V''(x=0.5, 0.83) = 10.463$ and $\fopt(0.83) = 2\pi/\sqrt{10.463} = 1.9425$.
The 5-point exact-pass yields a Lagrange residual at machine precision ($\sim 10^{-13}$),
establishing this as the v3.7-consistent confirmed value of $\fopt(0.83)$.

\subsubsection*{(b) cascade update ($\cmem$, $\chi_F$, $\Vxi$)}

The cascade from $\fopt(0.83) = 1.9163 \to 1.9425$ ($+1.37\%$) updates the following universal
quantities:

\begin{center}\small
\begin{tabular}{@{}llll@{}}
\toprule
Quantity & v4.8 v1 & v3 (deg-4 confirmed) & drift \\
\midrule
...
Since $\Vxi \propto \cmem^6$, the $\Vxi$ drift is about 6 times the $\cmem$ drift. The three
quantities marked with ${\dagger}$ (2-pt artifact) in v2 are promoted to deg-4-confirmed values
in v3, and the ${\dagger}$ markers are removed.

\subsubsection*{(c) Universal $\Lambdauv(c)$ complete retraction (F2 galaxy-specificity)}

The v3.7 decomposition F2: $\msigma(c, \mathrm{galaxy}) = \sqrt{V''(x=0.5, c)}/\tau_{\mathrm{dyn}}(\mathrm{galaxy})$
has $\tau_{\mathrm{dyn}}$ galaxy-specific, so $\Lambdauv = \msigma \cdot \clt^2$ inherently
contains a galaxy-dependent factor and is not well-defined as a c-only universal function.

v4.8 v1 presented $\Lambdauv(0.83) = 9.549 \times 10^{-49}$ J as a two-point $\msigma$ linear
extrapolation universal value, which v2 marked as a ${\dagger}$ artifact.
\textbf{v3 completely retracts universal $\Lambdauv$}:

\begin{itemize}
\item The $\Lambdauv(0.83)$ row is removed from Appendix~A.
\item The universal $\Lambdauv(0.83)$ description in \S 3.3 is retracted (per-galaxy table only).
\item The only legitimate $\Lambdauv$ numerical anchors are the three per-galaxy values in
      \S 3.3 (IC 2574 / NGC 3198 / NGC 2841).
\end{itemize}

This retraction is a structural decision directly following from F2.

\subsubsection*{(d) Invariants (unchanged in v3)}

The per-galaxy M1 anchor $\aPT^{\mathrm{upper}}(\mathrm{NGC~3198}, \Vxi) = 1.76 \times 10^{-51}$
is unchanged from v2. The 3-galaxy range $[1.6\times10^{-53}, 1.8\times10^{-50}]$ is unchanged.
$\BFIRAS$, $\Imu^{S1}$, S2/S1 ratio, and $\taum^{\mathrm{lower}}$ are all unchanged since they
are $\cmem$-independent. The M3 closed-form identity
$\kmem \cdot \Nmode|_{\Vxi} = (2/9\pi) \cmem^2 \Lambdauv^2/(a_0^{5/2}\hbar^3)$ holds exactly
per-galaxy (ratio $= 1.0000$, MRT-invariant).

\subsubsection*{(e) Future update candidates}

Additional galaxy samples for $\cmem(c)$ and independent measurements of $\Lambdauv(c)$
(precision CMB spectral distortion measurements, PIXIE~\citep{Kogut2011}) provide the
fundamental path to improving per-galaxy primary framework precision.
...
primary gives $\aPT^{\mathrm{upper}}(\mathrm{NGC\,3198}, \Vxi) = 1.76 \times 10^{-51}$ as the anchor
value (the v4.8 v1 expression using ``universal $c_0 = 0.83$'' and the value $2.96 \times 10^{-53}$
is an artifact; this v2 erratum unifies to per-galaxy expression; 3-galaxy range
$[1.6\times10^{-53},\,1.8\times10^{-50}]$; see \S 7.3 and Appendix~A).
...
$\aPT^{\mathrm{upper}}$ range & $[1.6\times10^{-53},\, 1.8\times10^{-50}]$ & 3-galaxy ($\approx$3 orders) \\
...
\footnotesize\noindent
v3 erratum (this version) establishes the universal layer as deg-4 5-point Lagrange fit values
of $V''(x=0.5, c)$ combined with the closed form $\fopt(c) = 2\pi/\sqrt{V''(x=0.5, c)}$. Universal
$\Lambdauv(c)$ has been fully retracted due to F2 structural galaxy-specificity ($\msigma$ depends
on galaxy-specific $\tau_{\mathrm{dyn}}$; see \S 3.3 and \S 7.3). Per-galaxy anchor values are
in \S 3.3 table and M1 / M2 / M3 claims (established in v2 erratum). Details in companion documents
\texttt{c3\_contamination\_dependency\_map\_v1} and \texttt{arxiv\_v48\_en\_erratum\_v3}.
\normalsize
